# GetFileinfo

VB6 working copy of `GetFileinfo` from Dave Robinson's OneDrive Historical Dev `VB/Old` folder. Project title: Ge tFileinfo. Output: `GetFileInfo.exe`. `VersionCompanyName`: TUSK Technologies.

**Source last updated:** 2001-06-01 · **Language:** VB6 · **Target:** VB6 Win32 · **Output:** WinForms exe

_Note: original OneDrive LastWriteTime values were wiped to 2026-08-27 by a zip transfer; date above uses best available evidence (headers/copyright where helpful)._

## Solution structure

| Project | Language | Type | Purpose |
|---------|----------|------|---------|
| `GFI` (`GetFileInfo.vbp`) | VB6 | WinForms exe | Ge tFileinfo |

## How to open

Open the `.vbp` in Visual Basic 6.0 IDE:
- `GetFileInfo.vbp`

## Requirements

- Visual Basic 6.0 IDE
- Registered OCX/DLL dependencies referenced by the `.vbp` (may need to be installed separately):
  - `RICHTX32.OCX`
  - `TABCTL32.OCX`

## Attribution and provenance

Working copy from Dave Robinson's OneDrive Historical Dev folder `VB/Old/GetFileinfo`.
Company names in project files: TUSK Technologies.

## License

MIT © 2026 VaderConsulting for Dave Robinson's code. See `LICENSE`.
